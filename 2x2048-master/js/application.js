...
window.requestAnimationFrame(function () {
  var handleData = function (data) {
    ...
    if (data.connected) {
      ...
      window.connection.send({
        state: {
          grid: window.Game.grid.serialize(),
          currentPlayer: window.Game.currentPlayer,
          scores: window.Game.scores
        }
      });
    }
    ...
    if (!window.inGame) {
      if (peers[window.peerx]) {
        if (peers[window.peerx] !== window.connectionId) { // Check to make sure we aren't connecting to ourself
          ...
          window.connection = peer.connect(peers[window.peerx], {
            reliable: true
          });
  ...
  if (window.location.hash) {
    ...
    console.log('conecte hashing', conn)
